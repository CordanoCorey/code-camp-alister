...
The ordinary editor path is complete. A Service Operator recovery HTTP action is not exposed in this slice; recovery remains a documented exceptional path that must be added only with the existing Operator authorization and audit controls, rather than weakening ordinary Membership authorization. The bounded agenda returns at most 100 entries and reports `hasMore`; cursor traversal is deferred because the mobile default range is 60 days and no safe UI for more than 100 group plans was needed for this foundation.
